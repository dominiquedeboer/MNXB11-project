...
with open("datasets/smhi-opendata_1_53430_20231007_155558_Lund.csv", encoding="utf-8") as data_file:
    all_lines = data_file.readlines() #this path because working directory is home (from where python was launched).
...
panda_frame = pd.read_csv(StringIO(data_string), sep=";", low_memory= False) #low_memory so that type is not assumed in the beginning -> error
...
data_string = "".join(data_lines)
#converts it such that pandas can be used for cleanup
panda_frame = pd.read_csv(StringIO(data_string), sep=";", low_memory=False)
